...
routes.get('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new TiposClienteController().handle(routeContext)));

routes.post('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new CadastroTipoClienteController().handle(routeContext)));

routes.get('/ativos',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new TiposClienteAtivosController().handle(routeContext)));
