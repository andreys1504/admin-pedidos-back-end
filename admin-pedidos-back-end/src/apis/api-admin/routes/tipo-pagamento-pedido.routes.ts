...
routes.get('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new TiposPagamentoController().handle(routeContext)));

routes.post('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new CadastroTipoPagamentoPedidoController().handle(routeContext)));

routes.get('/ativos',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new TiposPagamentoAtivosController().handle(routeContext)));
