...
routes.get('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new TiposPedidosController().handle(routeContext)));

routes.post('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new CadastroTipoPedidoController().handle(routeContext)));

routes.get('/ativos',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new TiposPedidoAtivosController().handle(routeContext)));
