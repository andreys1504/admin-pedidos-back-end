...
routes.post('/',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new CadastroProdutoController().handle(routeContext)));

routes.get('/descricao/:descricao',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new ProdutosPorConteudoDescricaoController().handle(routeContext)));

routes.get('/cadastro-pedido',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new ProdutosParaCadastroPedidoController().handle(routeContext)));

routes.get('/para-edicao',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new ProdutosParaEdicaoController().handle(routeContext)));

routes.put('/:id',
    authorize(),
    catchErrorsRoute(async (routeContext: RouteContext) => await new EdicaoProdutoController().handle(routeContext)));
