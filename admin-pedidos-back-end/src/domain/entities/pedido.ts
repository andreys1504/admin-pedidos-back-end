...
export class Pedido extends EntityDomain {
  ...
  @Column({
    name: DatabaseTables.SITUACAO_EXTERNA_PEDIDO + "Id",
    type: "integer",
  })
  situacaoExternaPedidoId: number;

  @ManyToOne(
    (type) => SituacaoExternaPedido,
    (situacaoExternaPedido) => situacaoExternaPedido.pedidos
  )
  @JoinColumn({ name: DatabaseTables.SITUACAO_EXTERNA_PEDIDO + "Id" })
  situacaoExternaPedido: SituacaoExternaPedido;

  @Column({
    name: DatabaseTables.TIPO_PAGAMENTO_PEDIDO + "Id",
    type: "integer",
  })
  tipoPagamentoPedidoId: number | null;

  @ManyToOne(
    (type) => TipoPagamentoPedido,
    (tipoPagamento) => tipoPagamento.pedidos
  )
  ...
  @ManyToOne(
    (type) => UsuarioAdmin,
    (usuario) => usuario.pedidosResponsabilidade
  )
  ...
  confirmarPagamentoExterno() {
    this.situacaoExternaPedidoId =
      IdsSituacoesExternasPedidoConstante.EmAndamento;
