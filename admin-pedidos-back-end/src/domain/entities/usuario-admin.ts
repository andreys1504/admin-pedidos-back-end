...
export class UsuarioAdmin extends EntityDomain {
  ...
  editar(dados: {
    ...
    if (dados.senhaEditada) this.senha = gerarSenhaCodificada(dados.senha);
  ...
  alterarSenha(novaSenha: string) {
    this.senha = gerarSenhaCodificada(novaSenha);
    this.necessarioAlteracaoSenha = false;
  }

  static gerarSenha(senha: string) {
    return gerarSenhaCodificada(senha);
  }
...
export const gerarSenhaCodificada = (senha: string) => {
