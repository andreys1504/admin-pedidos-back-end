...
import { dataAAAAMMDD, gerarGuid } from "../../0-core/helpers";
import { IdsUsuariosAdminConstante } from "../constantes/usuario-admin/ids-usuarios-admin.constante";
import { IdsSituacoesExternasPedidoConstante } from "../constantes/situacao-externa-pedido/ids-situacoes-externas-pedido.constante";
import { IdsTiposPagamentoConstante } from "../constantes/tipo-pagamento/ids-tipos-pagamento.constante";
import { IdTiposPedidosConstante } from "../constantes/tipo-pedido/ids-tipos-pedidos.constante";
import { Entidade } from "../../0-core/dominio/entidades/entidade";
import { PedidoDadosEntrega } from "./pedido-dados-entrega";

@Entity(TabelasBancoDados.PEDIDO)
export class Pedido extends Entidade {
    ...
    @Column({ name: TabelasBancoDados.SITUACAO_EXTERNA_PEDIDO + "Id", type: 'integer' })
    situacaoExternaPedidoId: number;

    @ManyToOne(type => SituacaoExternaPedido, situacaoExternaPedido => situacaoExternaPedido.pedidos)
    @JoinColumn({ name: TabelasBancoDados.SITUACAO_EXTERNA_PEDIDO + "Id" })
    situacaoExternaPedido: SituacaoExternaPedido;

    @Column({ name: TabelasBancoDados.TIPO_PAGAMENTO_PEDIDO + "Id", type: 'integer' })
    tipoPagamentoPedidoId: number | null;

    @ManyToOne(type => TipoPagamentoPedido, tipoPagamento => tipoPagamento.pedidos)
    @JoinColumn({ name: TabelasBancoDados.TIPO_PAGAMENTO_PEDIDO + "Id" })
    tipoPagamentoPedido: TipoPagamentoPedido;

    @Column({ name: TabelasBancoDados.CLIENTE + "Id", type: 'integer' })
    clienteId: number;

    @ManyToOne(type => Cliente, cliente => cliente.pedidos)
    @JoinColumn({ name: TabelasBancoDados.CLIENTE + "Id" })
    cliente: Cliente;

    @Column({ name: TabelasBancoDados.TIPO_PEDIDO + "Id", type: 'integer' })
    tipoPedidoId: number;

    @ManyToOne(type => TipoPedido, tipoPedido => tipoPedido.pedidos)
    @JoinColumn({ name: TabelasBancoDados.TIPO_PEDIDO + "Id" })
